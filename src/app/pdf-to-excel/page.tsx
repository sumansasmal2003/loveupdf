...
import { FileSpreadsheet, File as FileIcon, Loader2, X, Sparkles, Download } from "lucide-react";
...
export default function PdfToExcelPage() {
  ...
  const convertToExcel = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("No tables found or conversion failed.");
      }
      ...
      alert("Conversion failed. Make sure your PDF contains readable, structured tables and not just images of data.");
    } finally {
  ...
  return (
    <div className="flex flex-col items-center flex-1 p-6 sm:p-12 md:p-20 max-w-4xl mx-auto w-full">
      <header className="text-center mb-10 w-full">
        <h1 className="text-3xl sm:text-4xl font-medium text-[#e3e3e3] mb-4">PDF to Excel</h1>
        <p className="text-[#c4c7c5]">Extract tables and data from your PDFs into perfectly formatted Excel spreadsheets.</p>
      </header>
            ...
            >
              ...
              <div className="flex items-center justify-between pb-6 border-b border-white/5 mb-6 z-10 w-full">
                <div className="flex items-center gap-4 overflow-hidden flex-1 min-w-0">
                  ...
                  <div className="flex flex-col overflow-hidden flex-1 min-w-0">
                    ...
                    <span className="text-sm text-[#8e918f]">{isProcessing ? "Extracting tables..." : "Ready for extraction"}</span>
                  </div>
                </div>
              ...
              <div className="flex items-start gap-3 p-4 bg-[#8ab4f8]/10 border border-[#8ab4f8]/20 rounded-xl mb-6">
                ...
                <p className="text-sm text-[#c4c7c5]">
                  <strong className="text-[#8ab4f8] font-medium">Smart Extraction:</strong> Our engine scans the geometry of the document. Each table found will be placed on its own dedicated tab in the Excel workbook.
                </p>
              </div>
              ...
              >
                {isProcessing ? <><Loader2 className="w-5 h-5 animate-spin" /> Processing...</> : <><Download className="w-5 h-5" /> Convert to XLSX</>}
              </button>
            </motion.div>
          </AnimatePresence>
        </div>
